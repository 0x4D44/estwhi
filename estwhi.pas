...
CONST
  ...
  ID_HARDSCORE = 462;    (* OPTIONS hard scoring *)
  ...
  TextCardSuits: ARRAY [1..4] OF CHAR = 'CDSH';
...
TYPE
  ...
  TMainWindow = OBJECT(TWindow)
    ...
    HardScore,                             (* Hard scoring enabled? *)
...
VAR
   ...
   RoundNo, BackNo, DealtCards, NoCards, LButDown,
   ...
   DebugEnabled: BOOLEAN;                 (* Is debugging enabled? *)
...
BEGIN
  ...
  ODS('Estimation Whist initialised');
...
BEGIN
  ...
  Str(ORD(HardScore), TextString);
  WritePrivateProfileString('Main Window', 'Hard score', TextString, INIFILENAME);
...
BEGIN
  ...
  HardScore := BOOL(GetPrivateProfileInt('Main Window', 'Hard score', 0, INIFILENAME));
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        IF (YPos[A] > (Attr.H - 95)) THEN YPos[A] := YPos[A] - Multiplier;
        ...
        IF ((YPos[A] > 200) AND (XPos[A] > 500)) THEN
        BEGIN
...
VAR
   A, TotalTricks, AvailableBonus: INTEGER;
BEGIN
  (* This procedure calculates all the scores at the end of a round *)

  (* Calculate the total number of tricks bid for *)
  TotalTricks := 0;
  FOR A := 1 TO NoPlayers DO
  BEGIN
    TotalTricks := TotalTricks + PlayCall[A];
  END;

  (* Now do the actual scoring *)
  FOR A := 1 TO NoPlayers DO
  BEGIN
    IF ((TotalTricks < DealtCards) AND (PlayCall[A] = 0) AND (HardScore)) THEN
    BEGIN
      AvailableBonus := 0;
    END
    ELSE
    ...
    BEGIN
      ...
      BEGIN
        Score[A] := Score[A] + AvailableBonus;
      END;
    ...
    BEGIN
      IF (PlayCall[A] = Tricks[A]) THEN
      BEGIN
        Score[A] := Score[A] + AvailableBonus + (Tricks[A] * Tricks[A])
      END
      ELSE
      BEGIN
        Score[A] := Score[A] + Tricks[A];
...
VAR
   A, B, C, Top, MaxCard, MaxPos, TopCard: INTEGER;
   MoveUp: BOOLEAN;
BEGIN
  FOR A := 1 TO NoPlayers DO
  BEGIN
    ...
    (* Rearrange the aces as they are high *)
    B := 1;
    FOR C := 0 TO 3 DO
    BEGIN
      WHILE (HandCards[A, B] < (1 + (13*C))) DO
      BEGIN
        B := B + 1;
      END;

      IF (HandCards[A, B] = (1 + (13*C))) THEN
      BEGIN
        WHILE ((B < NoCards) AND (HandCards[A, B+1] < (1 + (13*(C+1))))) DO
        BEGIN
          HandCards[A, B] := HandCards[A, B+1];
          B := B + 1;
        END;
        HandCards[A, B] := 1 + (13*C);
      END;
    END;
...
BEGIN
 ...
 (*     IF ((HardScore) AND (TotalTricks < DealtCards) AND (PlayCall[A] = 0)) THEN
      BEGIN
        StrCat(TextString, ' (n)    ');
      END
      ELSE
    ...
    CASE Trump OF
      ...
      3 : DrawSmallBMP(DC, 285, 80, SpdePic, 1);
      4 : DrawSmallBMP(DC, 285, 80, HertPic, 1);
    END;
...
PROCEDURE TMainWindow.ODS(AStr: PChar);
BEGIN
...
BEGIN
  ...
  Chk3 := New(PCheckBox, InitResource(@self, ID_HARDSCORE));
END;
...
BEGIN
  ...
  IF (PMainWindow(Parent)^.HardScore)    THEN Chk3^.SetCheck(BF_CHECKED);
END;
...
BEGIN
  ...
  IF (Chk3^.GetCheck = BF_CHECKED) THEN
    PMainWindow(Parent)^.HardScore := TRUE
  ELSE
    PMainWindow(Parent)^.HardScore := FALSE;
